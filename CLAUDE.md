...
The README and `docs/PROJECT.md` describe the AI as "OpenAI-compatible," but **the current code does not.** `ai_analyzer.js` `postChatCompletion()` POSTs to `${ai.baseURL}/v1/messages` with header `anthropic-version: 2023-06-01`, a top-level `system` field, `temperature: 0.1`, and parses `json.content[].text` blocks. `config/config.json` sets `ai.baseURL: "https://open.bigmodel.cn/api/anthropic"` and `ai.model: "glm-5.1"` — i.e. Zhipu BigModel's **Anthropic-compatible** endpoint. Treat the docs' OpenAI/`paas/v4`/`GLM-5V-Turbo` references as stale.
